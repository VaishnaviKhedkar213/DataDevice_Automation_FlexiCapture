...
public class P01 {
    ...
    public void processAllTasks() throws InterruptedException {
        ...
        // Step 2: Collect Task URL + Task ID + Batch Name FROM SAME ROW
        List<WebElement> rows = driver.findElements(
            By.cssSelector("#tasks_table tbody tr")
        );

        List<String> taskUrls = new java.util.ArrayList<>();
        List<String> taskIds = new java.util.ArrayList<>();
        List<String> batchNames = new java.util.ArrayList<>();

        for (WebElement row : rows) {

            // Task ID + URL
            WebElement taskLink = row.findElement(
                By.cssSelector("td:first-child span.taskLink")
            );
            taskIds.add(taskLink.getText().trim());
            taskUrls.add(taskLink.getAttribute("data-href"));

            // Batch name (4th column)
            WebElement batchCell = row.findElements(By.cssSelector("td")).get(3);
            batchNames.add(batchCell.getText().trim());
        }
        ...
        for (int i = 0; i < taskUrls.size(); i++) {
            ...
            System.out.println("\n========================================");
            System.out.println(
                "===== Processing Task " + (i + 1) + " of " + taskUrls.size() +
                " → Batch → " + batchNames.get(i) +
                " → " + taskIds.get(i) +
                " ====="
            );
            System.out.println("========================================\n");
            ...
            // Return to queue
            driver.get(
                "https://afc5.datadevice.com.au/FlexiCapture12/Verification/Trescon/Tasks?projectId=58&roleid=702&stageType=500&queueName=Verification"
            );
            Thread.sleep(2000);
...
public void validateFieldDynamically(String errorText) {

    try {
        // 🔹 STEP 1: Extract only field name properly

        String fieldName = "";

        // Case 1: If comma rule error (GSTAmount,InvoiceAmount,...)
        if (errorText.contains(",")) {
            fieldName = errorText.split(",")[0].trim();
        }
        // Case 2: If hyphen field like ABN-Vendor
        else if (errorText.contains("ABN-Vendor")) {
            fieldName = "ABN-Vendor";
        }
        ...
        // 🔹 Correct XPath using title attribute
        By fieldLocator = By.xpath(
                "//div[@contenteditable='true' and @title='" + fieldName + "']"
        );

        WebElement fieldElement = wait.until(
                ExpectedConditions.visibilityOfElementLocated(fieldLocator)
        );

        String value = fieldElement.getText().trim();
        ...
        System.out.println("❌ Could not validate: " + errorText);
    }
}
